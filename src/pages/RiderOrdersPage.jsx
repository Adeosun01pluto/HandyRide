...
//   // Load rider profile from localStorage + auth listener
//   useEffect(() => {
//     let rider = null;
//     try {
...
//       alert(
//         "Missing permissions or network error. Check that this rider is approved."
//       );
//     }
//   };
...
//               {t.label}
//               <span
//                 className={`ml-2 inline-flex items-center justify-center w-6 h-6 text-xs font-bold rounded-full ${t.pill} text-white`}
//               >
//                 {t.count}
//               </span>
//             </button>
...
//               orders={mine}
//               accent="amber"
//               title="Awaiting user payment"
//             />
//           ) : (
//             <div className="text-center py-20 text-gray-500">
//               You have no unpaid accepted orders.
//             </div>
//           ))}
...
//           currentRider?.email &&
//           o.bid.counterBy === currentRider.email;
//         const userDeclinedYourCounterFlag = userDeclinedYourLastCounter(
//           o?.bid,
//           currentRider?.email
//         );
...
//                   Counter sent — waiting for the user response
//                 </div>
//               )}
...
//           <div
//             className={`inline-block px-3 py-1 rounded-full border text-xs font-semibold ${accentBg}`}
//           >
//             {title}
//           </div>
...
//             Updated:{" "}
//             {tsToDate(
//               r.updatedAt || r.acceptedAt || r.createdAt
//             )?.toLocaleString?.() || "—"}
//           </p>
//         </div>
//       ))}
...
export default function RiderOrdersPage() {
  ...
  useEffect(() => {
    ...
    // 🔐 Extra guard: only allow real riders
    const isRider = rider && (rider.role === "rider" || rider.isRider === true);
    if (!isRider) {
      // Hard redirect so they can't stay on the page
      window.location.href = "/rider-login";
      return;
    }
  ...
  const handleLogout = async () => {
    try {
      ...
      console.error("rider logout error:", e);
    }
  ...
  const handleAcceptOrder = async (order) => {
    ...
    const riderVehicleType = (currentRider.vehicleType || "").toLowerCase();
    const orderRideType = (order.rideType || "").toLowerCase();

    if (!riderVehicleType) {
      alert(
        "Your vehicle type is not set. Please contact admin or update your rider profile."
      );
      return;
    }

    // If user picked a specific type (Bike/Car), rider must match that type
    if (
      orderRideType &&
      orderRideType !== "any" &&
      riderVehicleType !== orderRideType
    ) {
      alert(
        `This order is for '${order.rideType}' rides, but your profile vehicle type is '${currentRider.vehicleType}'. You can only accept matching rides.`
      );
      return;
    }
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-red-100 text-xs flex flex-col sm:flex-row sm:items-center sm:gap-1">
                      <span>
                        Vehicle:{" "}
                        <span className="font-semibold">
                          {currentRider.vehicleType || "Not set"}
                        </span>
                      </span>
                ...
                <button
                  ...
                  className="text-xs font-semibold text-white bg-red-500/40 hover:bg-red-500/60 px-3 py-1.5 rounded-full border border-white/30"
                >
                  Logout
                </button>
              </div>
        ...
        return (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-red-500 to-orange-500 p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-white/80 text-xs">
                    {o.rideType || "Any"} • {o.vibe || "Any vibe"}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              {/* Extra trip details */}
              <div className="text-xs text-gray-600 space-y-1 bg-gray-50 rounded-xl p-3">
                <div>
